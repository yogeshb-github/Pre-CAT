...
void free_list()
{
    int value;             // true      true       true     true     true        false 
    ...
    printf("\n all node are deleted");
    return;
}

int del_at_pos(int position)
{
    node_t *temp=NULL, *trav=NULL;
    int i, value;
    if( position==1)
    {
        value= del_first();
    ...
    {
        trav= head;  // assign addrees of 1st node to trav
